...
@section('title', 'Families - Shajara')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        <form action="{{ route('families.index') }}" method="GET" class="d-flex">
            <input type="text" name="search" class="form-control me-2" placeholder="Search families..." value="{{ request('search') }}">
            <button type="submit" class="btn btn-outline-secondary">Search</button>
        </form>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createFamilyModal">
            <i class="fas fa-plus me-1"></i> New Family
        </button>
    </div>
</div>
            ...
            <div class="card h-100 shadow-sm">
                <div class="card-body">
                    ...
                    <p class="card-text text-muted">
                        {{ $family->people_count }} family members
                    </p>
    ...
    @empty
        <div class="col-12">
            <div class="alert alert-info text-center py-5">
                <h3>No families found yet.</h3>
                <p>Start by creating your first family!</p>
            </div>
        </div>
    @endforelse
</div>
        ...
        <form action="{{ route('families.store') }}" method="POST">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="name" class="form-label">Family Name (e.g. Usmonovlar)</label>
                        <input type="text" name="name" class="form-control" id="name" required placeholder="Enter family surname">
                    </div>
                </div>
